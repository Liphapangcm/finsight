...
def render_kpi_cards(kpis: FinancialKPIs):
    ...
    flow_badge = (
        _badge("b-good", "Surplus") if flow >= 0 else _badge("b-bad", "Deficit ⚠")
    )
    ...
    cards = [
        ("DEBT-TO-INCOME", f"{dti:.1f}%", COLORS["navy"], dti_badge),
        ("SAVINGS RATE", f"{sr:.1f}%", COLORS["navy"], sr_badge),
        ("MONTHLY CASH FLOW", f"M{flow_sign}{flow:,.0f}", flow_color, flow_badge),
        ("EXPENSE RATIO", f"{exp:.1f}%", COLORS["navy"], exp_badge),
        (
            "MONTHLY INCOME",
            f"M{kpis.monthly_income:,.0f}",
            COLORS["navy"],
            _badge("b-info", "Reported"),
        ),
        ("TOTAL EXPENSES", f"M{kpis.total_expenses:,.0f}", COLORS["navy"], exp_badge),
    ]
