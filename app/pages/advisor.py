...
def render_advisor():
    ...
    if not result:
        st.info("Complete your credit assessment first. The advisor uses "
    ...
    if not api_key:
        st.warning("ANTHROPIC_API_KEY not configured. "
    ...
    try:
        from core.ai_advisor import (
            ChatMessage, get_advisor_response,
            get_opening_message, get_suggested_questions,
        )
    except ImportError:
        st.error("AI advisor module not found. "
                 "Ensure core/ai_advisor.py exists.")
        return

    score_color = {
        'Poor': COLORS['danger'], 'Fair': COLORS['warning'],
        'Good': COLORS['good'],   'Excellent': COLORS['teal'],
    ...
    user_input = st.chat_input(
        "Ask your advisor anything about your finances...")
    ...
    # Footer
    st.markdown("<hr style='margin:1.5rem 0;border-color:#E5E7EB;'>",
                unsafe_allow_html=True)
