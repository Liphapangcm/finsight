...
def _nav_buttons(
    back_key: str, next_key: str, next_label: str, back_step: int, next_action=None
):
    """
    ...
    with col_next:
        if st.button(next_label, key=next_key, use_container_width=True):
            if next_action:
                next_action()
...
def render_step1():
    ...
    _trust()
    st.markdown(
        '<div class="form-section">Personal Information</div>', unsafe_allow_html=True
    )
    ...
    if st.button("Continue →", key="s1_next", use_container_width=True):
...
def render_step2():
    _step_indicator(2)
    st.markdown('<div class="form-section">Income</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        '<div class="form-section">Monthly Expenses</div>', unsafe_allow_html=True
    )
    ...
    with col_back:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
    with col_next:
        if st.button("Continue →", key="s2_next", use_container_width=True):
            st.session_state.update(
                {
...
def render_step3():
    _step_indicator(3)
    st.markdown('<div class="form-section">Debt & Credit</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        '<div class="form-section">Savings & Mobile Money</div>', unsafe_allow_html=True
    )
    ...
    with col_back:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
    with col_submit:
        if st.button("Calculate My Score →", key="s3_submit", use_container_width=True):
            st.session_state.update(
                {
