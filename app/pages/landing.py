...
def render_landing():
    ...
    with col2:
        if st.button("Get My Credit Score →", key="hero_cta", use_container_width=True):
    ...
    steps = [
        (
            "01",
            "Enter Your Details",
            "Fill in your income, expenses, and debt. No bank login or ID required.",
            COLORS["blue"],
        ),
        (
            "02",
    ...
    with col2:
        if st.button(
            "Start Free Assessment →", key="bottom_cta", use_container_width=True
        ):
            st.session_state["page"] = "assessment"
            st.rerun()
