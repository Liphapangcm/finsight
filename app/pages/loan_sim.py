...
def render_loan_simulator():
    ...
    if not result:
        st.info("Complete your credit assessment first to use the loan simulator.")
    ...
    # ── Input panel ───────────────────────────────────────────────
    st.markdown('<div class="fs-card fs-card-accent">', unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.caption(f"{loan_term} months = {loan_term // 12} yr {loan_term % 12} mo")
    ...
    sim = simulate_loan(
        ...
        monthly_savings=result.kpis.savings_rate / 100 * result.kpis.monthly_income,
        total_debt=result.kpis.debt_to_income / 100 * result.kpis.monthly_income,
        has_defaulted=any("default" in r.title.lower() for r in result.recommendations),
    )
    ...
    with col2:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-delta
                 {
                "kpi-positive" if sim.affordability.is_affordable else "kpi-negative"
            }">
                {"Affordable" if sim.affordability.is_affordable else "Too High"}
            </div>
        </div>
        """,
            unsafe_allow_html=True,
    ...
    with tab2:
        st.markdown(
            "##### Products you qualify for based on your score and requested amount"
        )
        for product in sim.eligible_products:
            bg = "#E8F5E9" if product.eligible else "#FAFAFA"
            color = COLORS["success"] if product.eligible else COLORS["muted"]
            st.markdown(
                f"""
    ...
    with tab3:
        ...
        balance_data = pd.DataFrame(
            [{"Month": s.month, "Remaining Balance": s.balance} for s in sim.schedule]
        )
        fig2 = px.line(
            balance_data,
            x="Month",
