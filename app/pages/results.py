...
def render_results():
    ...
    with col_detail:
        ...
        with r1c1:
            if st.button(
                "🏦  Loan Simulator", key="btn_loan", use_container_width=True
            ):
                st.session_state["page"] = "loan_simulator"
                st.rerun()
        with r1c2:
            if st.button("🤖  AI Advisor", key="btn_advisor", use_container_width=True):
        ...
        with r2c1:
            ...
            if st.button(
                "↺  New Assessment", key="btn_reassess", use_container_width=True
            ):
                for k in [
                    "result",
                    "form_step",
        ...
        with r2c2:
            ...
            if st.button("⬇  PDF Report", key="btn_pdf", use_container_width=True):
