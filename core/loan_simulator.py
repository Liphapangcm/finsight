...
LOAN_PRODUCTS = [
    {
        ...
        "description": "Short-term emergency finance. Available to most applicants.",
    },
    {
        "name": "Personal Loan",
    ...
    },
    {
        ...
        "max_term": 60,
        "base_rate": 0.18,
        "description": "For registered small businesses with trading history.",
    },
    {
...
def _approval_probability(
    credit_score: int, dti: float, is_affordable: bool, has_defaulted: bool
) -> float:
    """
...
def _approval_label_color(prob: float) -> tuple[str, str]:
    """Return a human label and hex color for an approval probability."""
    if prob >= 0.70:
        return "Likely Approved", "#43A047"
    if prob >= 0.45:
        return "Possible — Not Guaranteed", "#FB8C00"
    return "Unlikely to be Approved", "#E53935"
...
def _compute_affordability(
    ...
    base_rate_guess: float = 0.20,
) -> tuple[AffordabilityResult, float]:
    """Compute affordability result and effective monthly payment estimate.

    Returns (AffordabilityResult, monthly_payment_estimate)
    """
    ...
    # Max affordable loan given term using base_rate_guess
    if max_monthly > 0:
        r = base_rate_guess / 12
        if r > 0:
            max_loan = (
                max_monthly * ((1 + r) ** loan_term - 1) / (r * (1 + r) ** loan_term)
            )
        else:
    ...
    # Use effective rate guess to compute a monthly payment for requested loan
    eff_rate = _effective_rate(base_rate_guess, 700) if income <= 0 else _effective_rate(base_rate_guess, 650)
    # Note: caller will compute final monthly payment using actual credit score
    if loan_term > 0:
        monthly_pmt_est = _monthly_payment(loan_amount, eff_rate, loan_term)
    else:
        monthly_pmt_est = 0.0

    # New DSR estimate (very approximate)
    new_dsr = 0.0
    is_affordable = (monthly_pmt_est <= max_monthly) and (new_dsr < 0.50)

    afford_msg = (
        f"M{monthly_pmt_est:,.0f}/month fits within your budget."
        if is_affordable
        else f"M{monthly_pmt_est:,.0f}/month exceeds your safe repayment capacity of M{max_monthly:,.0f}/month."
    )
    ...
    return affordability, monthly_pmt_est


def _build_schedule(
    principal: float, annual_rate: float, term_months: int, monthly_pmt: float
...
def _check_product_eligibility(
    ...
    if credit_score < product["min_score"]:
        ...
        reason = (
            f"❌ Requires a minimum score of {product['min_score']}. "
            f"You need +{gap} points."
        )
    elif loan_amount > product["max_amount"]:
        eligible = False
        reason = f"❌ Maximum loan for this product is M{product['max_amount']:,.0f}."
...
def _generate_tips(
    result: "SimulationResult", credit_score: int, has_defaulted: bool
) -> List[str]:
    tips = []
...
def simulate_loan(
    ...
    # Affordability and monthly payment estimate
    affordability, monthly_pmt = _compute_affordability(
        loan_amount,
        loan_term,
        monthly_income,
        total_expenses,
        monthly_savings,
        base_rate_guess=0.20,
    )

    # Compute actual effective rate and monthly payment using the user's credit score
    eff_rate = _effective_rate(0.20, credit_score)
    monthly_pmt = _monthly_payment(loan_amount, eff_rate, loan_term)

    # New DTI after adding this loan (use income and total_debt)
    income = monthly_income + 1e-9
    existing_dsr = total_debt * 0.03
    new_dsr = (existing_dsr + monthly_pmt) / income
    # update affordability fields that depend on accurate DSR
    affordability.debt_service_ratio = round(new_dsr, 4)
    affordability.is_affordable = (monthly_pmt <= affordability.max_affordable_monthly) and (
        new_dsr < 0.50
    )
    affordability.affordability_message = (
        f"M{monthly_pmt:,.0f}/month fits within your budget."
        if affordability.is_affordable
        else f"M{monthly_pmt:,.0f}/month exceeds your safe repayment capacity of M{affordability.max_affordable_monthly:,.0f}/month."
    )

    # Approval probability
    dti = total_debt / income
    prob = _approval_probability(credit_score, dti, affordability.is_affordable, has_defaulted)
    label, color = _approval_label_color(prob)
