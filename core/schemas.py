...
class AssessmentInput:
    ...
    def validate(self) -> List[str]:
        ...
        total_exp = (self.housing_expense + self.food_expense +
                     self.transport_expense + self.utilities_expense +
                     self.other_expense)
        if total_exp > self.monthly_income * 3:
            errors.append(
                "Total expenses are more than 3× your income. Please check."
            )
        ...
        if self.mobile_money_frequency not in [m.value for m in MobileMoneyFrequency]:
            errors.append(f"Invalid mobile money frequency: {self.mobile_money_frequency}")
