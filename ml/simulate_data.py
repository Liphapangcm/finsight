...
def _generate_profile() -> dict:
    ...
    if employment == "unemployed":
        default_prob += 0.25
    if total_debt / (income + 1) > 1.5:
        default_prob += 0.20
    has_defaulted = int(np.random.random() < default_prob)

    # Payment regularity
    if has_defaulted:
        pay_reg = np.random.choice(["always", "sometimes", "rarely"], p=[0.1, 0.4, 0.5])
    else:
        pay_reg = np.random.choice(["always", "sometimes", "rarely"], p=[0.6, 0.3, 0.1])

    # Savings
    has_savings = int(np.random.random() < 0.45)
    savings = income * np.random.uniform(0.02, 0.25) if has_savings else 0.0
...
def _compute_features(p: dict) -> dict:
    ...
    essential = p["housing_expense"] + p["food_expense"] + p["utilities_expense"]
    ...
    emp_map = {"employed": 3, "self_employed": 2, "student": 1, "unemployed": 0}
