...
def _get_styles():
    ...
    return {
        'title': ParagraphStyle(
            ...
            textColor=NAV, alignment=TA_CENTER, spaceAfter=4,
        ),
        ...
        'section_header': ParagraphStyle(
            'section_header', fontSize=12, fontName='Helvetica-Bold',
            textColor=NAV, spaceBefore=14, spaceAfter=6,
        ),
        'body': ParagraphStyle(
        ...
        'body_muted': ParagraphStyle(
            'body_muted', fontSize=8, fontName='Helvetica',
            textColor=MUTED, leading=12, spaceAfter=3,
        ),
        'rec_title': ParagraphStyle(
            'rec_title', fontSize=10, fontName='Helvetica-Bold',
            textColor=NAV, spaceAfter=3,
        ),
        'rec_body': ParagraphStyle(
            'rec_body', fontSize=8.5, fontName='Helvetica',
            textColor=colors.HexColor('#374151'),
            leading=13, spaceAfter=2,
        ),
        'impact': ParagraphStyle(
            'impact', fontSize=8, fontName='Helvetica-Bold',
            textColor=SUCCESS, spaceAfter=0,
        ),
        'score_num': ParagraphStyle(
            'score_num', fontSize=48, fontName='Helvetica-Bold',
            alignment=TA_CENTER, spaceAfter=0,
        ),
        'score_band': ParagraphStyle(
            'score_band', fontSize=14, fontName='Helvetica-Bold',
...
def _build_header(styles, result: ScoreResult) -> list:
    ...
    header_data = [[
        Paragraph('<font color="#00BFA6"><b>Fin</b></font>'
                  '<font color="white"><b>Sight</b></font>',
                  ParagraphStyle('logo', fontSize=20,
                                 fontName='Helvetica-Bold')),
        Paragraph(f'Credit Score Report<br/>'
                  f'<font size="8" color="#94A3B8">'
                  f'Generated: {now} · ID: {result.assessment_id[:12]}'
                  f'</font>',
                  ParagraphStyle('hdr_right', fontSize=11,
                                 fontName='Helvetica-Bold',
                                 textColor=WHITE,
                                 alignment=TA_RIGHT)),
    ]]
...
def _build_score_block(styles, result: ScoreResult) -> list:
    ...
    score_para = Paragraph(
        f'<font color="{color.hexval()}">{sc}</font>',
        styles['score_num']
    )
    band_para  = Paragraph(
        f'<font color="{color.hexval()}">{band.upper()}</font>',
        styles['score_band']
    )
    range_para = Paragraph(
        'Score Range: 300 – 850 &nbsp;|&nbsp; '
        f'Risk Level: <b>{risk}</b>',
        ParagraphStyle('range', fontSize=9, fontName='Helvetica',
                       textColor=MUTED, alignment=TA_CENTER)
    )
    ...
    block_data = [[score_para], [band_para], [range_para],
                  [Spacer(1,6)], [bar]]
...
def _build_kpi_table(styles, result: ScoreResult) -> list:
    ...
    def kpi_cell(label, value, status, status_color):
        return [
            Paragraph(f'<b>{value}</b>',
                      ParagraphStyle('kv', fontSize=14,
                                     fontName='Helvetica-Bold',
                                     textColor=NAV, alignment=TA_CENTER)),
            Paragraph(label,
                      ParagraphStyle('kl', fontSize=7.5,
                                     fontName='Helvetica',
                                     textColor=MUTED, alignment=TA_CENTER)),
            Paragraph(f'<font color="{status_color}">{status}</font>',
                      ParagraphStyle('ks', fontSize=8,
                                     fontName='Helvetica-Bold',
                                     alignment=TA_CENTER)),
        ]
    ...
    cells = [
        kpi_cell("Debt-to-Income", f"{dti:.1f}%",
                 "Healthy" if dti<35 else "Moderate" if dti<50 else "High",
                 dti_c),
        kpi_cell("Savings Rate", f"{sr:.1f}%",
                 "Strong" if sr>=10 else "Low" if sr>=3 else "None", sr_c),
        kpi_cell("Monthly Cash Flow",
                 f"M{'+' if flow>=0 else ''}{flow:,.0f}",
                 "Surplus" if flow>=0 else "Deficit", fl_c),
        kpi_cell("Expense Ratio", f"{exp:.1f}%",
                 "Healthy" if exp<70 else "Watch" if exp<90 else "High", ex_c),
    ]
    ...
    tbl = Table([row], colWidths=[46*mm]*4,
                hAlign='CENTER')
    tbl.setStyle(TableStyle([
        ('ALIGN',   (0,0),(-1,-1),'CENTER'),
        ('VALIGN',  (0,0),(-1,-1),'MIDDLE'),
...
def _build_shap_table(styles, result: ScoreResult) -> list:
    ...
    rows = [[
        Paragraph('<b>Factor</b>',
                  ParagraphStyle('th', fontSize=8.5,
                                 fontName='Helvetica-Bold',
                                 textColor=WHITE)),
    ...
    for name, val in pairs:
        ...
        rows.append([
            ...
            Paragraph(f'{val:+.1f}',
                      ParagraphStyle('val', fontSize=9,
                                     fontName='Helvetica-Bold',
                                     textColor=NAV,
                                     alignment=TA_CENTER)),
            Paragraph(f'<font color="{eff_c}">{effect}</font>',
                      ParagraphStyle('eff', fontSize=8.5,
                                     fontName='Helvetica',
                                     alignment=TA_CENTER)),
        ])
...
def _build_recommendations(styles, result: ScoreResult) -> list:
    ...
    for rec in result.recommendations:
        ...
        inner = Table([
            [Paragraph(f'<b>{rec.priority}. {rec.title}</b>  '
